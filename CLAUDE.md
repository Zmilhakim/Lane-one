...
This repository is one token, launched through the launchpad in
[toollpad](https://github.com/Zmilhakim/toollpad). It is not part of it: the
launchpad supplies a pool and a rate, and everything else here is this token's
own.
...
`npm run wallets` in the launchpad's `contracts/` refuses to run in a hosted
session on purpose:
...
Every figure printed on art or a page comes from one recorded place — here that
is `launchpad.json`, which names the repository and commit its figures were
copied from, and the page reads the live rate off the chain besides. No
